...
// GET /api/gateway/connectors - List available DEX connectors
export async function GET() {
    try {
        const response = await fetch(`${API_URL}/gateway/connectors`, {
        ...
        console.error('[API] Failed to fetch Gateway connectors:', error);
        return NextResponse.json(
            { error: error.message || 'Failed to fetch Gateway connectors' },
            { status: 500 }
        );
    }
}
