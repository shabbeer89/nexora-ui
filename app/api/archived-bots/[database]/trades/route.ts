...
// GET /api/archived-bots/[database]/trades - Get trades from archived bot
export async function GET(
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/archived-bots/${encodeURIComponent(database)}/trades`, {
        ...
        console.error('[API] Failed to fetch archived bot trades:', error);
        return NextResponse.json(
            { error: error.message || 'Failed to fetch archived bot trades' },
            { status: 500 }
        );
    }
}
