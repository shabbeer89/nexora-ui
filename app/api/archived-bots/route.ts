...
// GET /api/archived-bots - List archived bot databases
export async function GET() {
    try {
        const response = await fetch(`${API_URL}/archived-bots/databases`, {
        ...
        console.error('[API] Failed to fetch archived bots:', error);
        return NextResponse.json(
            { error: error.message || 'Failed to fetch archived bots' },
            { status: 500 }
        );
    }
}
